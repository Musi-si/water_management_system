...
class H2O_management:
    ...
    def filter_admin_usage( start_date, end_date, client_id, max_quantity, min_quantity ):
        try:
            usages_ = Usage.objects.all()
            if start_date and end_date:
                usages = usages_.filter( date__range = [ start_date, end_date ] )
            if client_id:
                usages = usages_.filter( client_id = client_id )
            if max_quantity:
                usages = usages_.filter( quantity__lte = max_quantity )
            if min_quantity:
                usages = usages_.filter( quantity__gte = min_quantity )
    ...
    def filter_client_usage( id, start_date, end_date, max_quantity, min_quantity ):
        try:
            usages_ = Usage.objects.filter( client_id = id )
            if start_date and end_date:
                usages = usages_.filter( date__range = [ start_date, end_date ] )
            if max_quantity:
                usages = usages_.filter( quantity__lte = max_quantity )
